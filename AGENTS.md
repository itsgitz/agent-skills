...
- **Missing skills** — never block, never silently install. Plan agents (no bash) inform in one line, print the `npx skills add …` command, write it into the plan doc's "Suggested skills" note, and continue on the inline fallback. Build + combined agents (have bash) ask `(y/n)` before installing; on decline they continue and state what degrades (a declined `code-review` install means the work is reported unreviewed, not silently skipped).
